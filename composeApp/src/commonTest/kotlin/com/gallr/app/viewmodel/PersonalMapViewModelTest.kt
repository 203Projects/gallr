...
class PersonalMapViewModelTest {
    ...
    @Test
    fun `initial all state opens Seoul directly`() = runTest(dispatcher) {
        val exhibitions = MutableStateFlow<ExhibitionListState>(
        ...
        val viewModel = PersonalMapViewModel(
        ...
        assertEquals(listOf("seoul"), viewModel.uiState.value.resultExhibitions.map { it.id })
        assertEquals(25, viewModel.uiState.value.childSummaries.size)
    }

    @Test
    fun `same Seoul venue coordinate exposes every grouped exhibition`() = runTest(dispatcher) {
        val viewModel = PersonalMapViewModel(
            exhibitionsState = MutableStateFlow(
                ExhibitionListState.Success(
        ...
        backgroundScope.launch { viewModel.uiState.collect {} }
        advanceUntilIdle()
        val grouped = viewModel.uiState.value.projection.marks.single { it.itemIds.size == 2 }
        viewModel.selectMark(grouped.id)
        advanceUntilIdle()

        assertEquals(listOf("a", "b"), viewModel.uiState.value.selectedExhibitions.map { it.id })
        assertEquals("종로구", viewModel.uiState.value.selectedDistrictLabel)
    }

    @Test
    fun `selecting a district label focuses one of its exhibition marks`() = runTest(dispatcher) {
        val jongno = exhibition("jongno")
        val yongsan = exhibition("yongsan", latitude = 37.53, longitude = 126.97).copy(
            regionKo = "용산구",
            regionEn = "Yongsan-gu",
        )
        val viewModel = PersonalMapViewModel(
        ...
        backgroundScope.launch { viewModel.uiState.collect {} }
        advanceUntilIdle()
        val yongsanScope = viewModel.uiState.value.childSummaries.single { it.scope.labelKo == "용산구" }.scope
        viewModel.selectChildArea(yongsanScope.id)
        advanceUntilIdle()

        assertEquals("용산구", viewModel.uiState.value.selectedDistrictLabel)
        assertEquals(listOf("yongsan"), viewModel.uiState.value.selectedExhibitions.map { it.id })
    }
    ...
    fun `district editor toggles multiple districts and exposes their combined exhibition count`() =
        runTest(dispatcher) {
            ...
            backgroundScope.launch { viewModel.uiState.collect {} }
            advanceUntilIdle()
            val districts = viewModel.uiState.value.childSummaries.associateBy { it.scope.labelKo }
    ...
    @Test
    fun `district selection save remains on the direct Seoul map`() = runTest(dispatcher) {
        val viewModel = PersonalMapViewModel(
        ...
        backgroundScope.launch { viewModel.uiState.collect {} }
        advanceUntilIdle()
        val jongno = viewModel.uiState.value.childSummaries.single { it.scope.labelKo == "종로구" }.scope
        ...
        advanceUntilIdle()
        assertTrue(viewModel.uiState.value.selectedDistrictIds.isEmpty())
    ...
    fun `to visit mode contains saved unvisited items while visited stays empty before diary support`() =
        runTest(dispatcher) {
            val viewModel = PersonalMapViewModel(
                exhibitionsState = MutableStateFlow(
                    ExhibitionListState.Success(listOf(exhibition("saved"), exhibition("other", latitude = 37.59))),
                ),
            ...
            advanceUntilIdle()
            assertEquals(1, viewModel.uiState.value.projection.marks.sumOf { it.itemIds.size })

            viewModel.setMode(PersonalMapMode.VISITED)
            advanceUntilIdle()
            assertTrue(viewModel.uiState.value.projection.marks.isEmpty())
        }

    @Test
    fun `loading empty and generic city states preserve an equivalent map path`() = runTest(dispatcher) {
        ...
        exhibitions.value = ExhibitionListState.Success(
        ...
        assertEquals(listOf("busan"), viewModel.uiState.value.resultExhibitions.map { it.id })
        ...
        assertTrue(viewModel.uiState.value.resultExhibitions.isEmpty())
    }
