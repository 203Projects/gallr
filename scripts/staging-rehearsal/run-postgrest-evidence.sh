...
unset GALLR_POSTGREST_TARGET_GUARD GALLR_POSTGREST_TARGET_GUARD_SHA256
...
toolchain_path="${script_dir}/lib/reviewed-toolchain.sh"
...
[[ -f "${toolchain_path}" && ! -L "${toolchain_path}" ]] \
...
operator_manifest="${evidence_dir}/operator-manifest.txt"
# shellcheck source=lib/reviewed-toolchain.sh
source "${toolchain_path}"
gallr_read_reviewed_toolchain "${operator_manifest}" \
...
printf '%s\n' \
  ...
  "toolchain_helper_sha256=$(sha256_file "${toolchain_path}")" \
  "reviewed_node_sha256=${GALLR_REVIEWED_NODE_SHA256}" \
...
if [[ "${case_name}" == 'mutation' ]]; then
  if gallr_run_reviewed_node \
    ...
    "SUPABASE_URL=${supabase_url}" \
    "SUPABASE_ANON_KEY=${supabase_anon_key}" \
    -- "${integration_path}" >>"${evidence_path}" 2>&1; then
    postgrest_status=0
  else
    postgrest_status=$?
  fi
...
set -e
[[ "${postgrest_status}" -eq 0 ]] || fail "PostgREST ${case_name} evidence failed"
