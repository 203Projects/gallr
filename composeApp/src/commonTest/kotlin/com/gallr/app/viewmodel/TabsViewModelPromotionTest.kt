...
class TabsViewModelPromotionTest {
    ...
    fun `selected city loads paid placement through its separate repository`() = runTest(dispatcher) {
        ...
        val placement = PromotedExhibition(
            ...
            "아틀리에 한남", "Atelier Hannam", "서울", "Seoul", "용산구", "Yongsan-gu",
            "2026-08-08", "2026-09-14", null,
        )
    ...
    @Test
    fun `returning to a city reuses the session placement without spending the daily cap again`() = runTest(dispatcher) {
        var calls = 0
        val placement = PromotedExhibition(
            "promotion-one", "between-seasons", "계절 사이", "Between Seasons",
            "아틀리에 한남", "Atelier Hannam", "서울", "Seoul", "용산구", "Yongsan-gu",
