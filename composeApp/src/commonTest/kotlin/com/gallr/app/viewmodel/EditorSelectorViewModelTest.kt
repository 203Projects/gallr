...
class EditorSelectorViewModelTest {
    ...
    @Test
    fun `loadEditors success splits editors by isCurrentlyActive`() =
    ...
    @Test
    fun `loadEditors failure produces Error state`() =
        runTest(UnconfinedTestDispatcher()) {
            val repo =
                FakeEditorRepository(
    ...
    @Test
    fun `selector stays loading while exhibitions are loading for non-empty editor list`() =
        runTest(UnconfinedTestDispatcher()) {
            val repo =
                FakeEditorRepository(
    ...
    @Test
    fun `selector surfaces exhibition load error for non-empty editor list`() =
        runTest(UnconfinedTestDispatcher()) {
            val repo =
                FakeEditorRepository(
                    allEditorsResult = Result.success(listOf(editor("alice"))),
    ...
    @Test
    fun `retry reloads editors and exhibitions`() =
        runTest(UnconfinedTestDispatcher()) {
    ...
    @Test
    fun `exhibition counts join correctly against allExhibitions`() =
        runTest(UnconfinedTestDispatcher()) {
    ...
    @Test
    fun `exhibition counts use allExhibitions not filtered list - no filter leak`() =
        runTest(UnconfinedTestDispatcher()) {
    ...
    @Test
    fun `exhibition counts include only catalog-visible exhibitions`() =
        runTest(UnconfinedTestDispatcher()) {
            val alice = editor("alice")
            val exhibitions =
                listOf(
    ...
    @Test
    fun `selector hides editors with zero catalog-visible exhibitions`() =
        runTest(UnconfinedTestDispatcher()) {
            ...
            val repo =
                FakeEditorRepository(
                    allEditorsResult =
                        Result.success(
                            listOf(
                                activeWithExpiredOnly,
                                activeWithCurrent,
                                pastWithHiddenUpcomingOnly,
                                pastWithCurrent,
                            ),
                        ),
                )
    ...
    @Test
    fun `selector success can be empty when every editor has zero catalog-visible exhibitions`() =
        runTest(UnconfinedTestDispatcher()) {
            val alice = editor("alice")
    ...
    @Test
    fun `loadEditors is called on init`() =
        runTest(UnconfinedTestDispatcher()) {
    ...
    @Test
    fun `empty editor list produces Success with empty active and past`() =
        runTest(UnconfinedTestDispatcher()) {
            val repo = FakeEditorRepository(allEditorsResult = Result.success(emptyList()))
