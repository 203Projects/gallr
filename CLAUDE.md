...
## Recent Changes
- 043-android-editor-screen-fix: Editor selector + detail screens wrapped in Scaffold with WindowInsets.safeDrawing and colorScheme.background; EditorTopBar rewritten as Material3 TopAppBar (visual style unchanged). Android-only chrome fix; no behavior or data changes.
- 041-editor-hub: Migration 017 unifies `guest_editors` → `editors` table + seeds `gallr-editors` house row; replaces `is_editors_pick` + `guest_editor_id` with unified `editor_id` FK; GuestEditor model renamed to Editor with `isActive`/`activeFrom`/`activeTo`; new EditorRepository, EditorApiClient; new EditorSelectorScreen + EditorDetailScreen + EditorTopBar + EditorTile + EditorBanner composables; new EditorSelectorViewModel + EditorDetailViewModel (VMs take StateFlows directly); 12 new ViewModel tests; Apps Script sync updated (KNOWN_COLUMNS + FK rename + drop is_editors_pick branch); see `gas/README.md` for v1.5.x → v1.6 sheet migration tip (v1.6.0)
