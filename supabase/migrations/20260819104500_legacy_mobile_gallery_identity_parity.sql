-- Restore Seoul-to-Singapore mobile catalogue parity after gallery identity.
--
-- Background
-- ----------
-- `20260814010000_public_gallery_identity.sql` added `gallery_id` to
-- `public.exhibition_catalog_v2` on Seoul. The canonical checksum is derived
-- from `to_jsonb(row) - 'content_checksum_sha256'`, so it covers every column
-- of the row. Seoul began hashing 34 fields while the compatibility project
-- still hashed 33.
--
-- Every mirrored row therefore failed the receiver's checksum comparison with
-- `legacy_mobile_catalog_canonical_v2_checksum_mismatch`, the guarded apply
-- aborted, and installed pre-1.7.7 mobile clients reading the compatibility
-- project stopped receiving new exhibitions. The guard behaved correctly: it
-- refused to trust a snapshot it could not verify.
--
-- `gallery_id` is a random UUID assigned by the Seoul directory, not a value
-- derivable from exhibition content, and the compatibility project has no
-- gallery directory of its own. The identity must therefore be carried in the
-- snapshot rather than recomputed locally.
--
-- This migration is additive and safe to apply to both projects in the pair:
-- on the compatibility project it installs the column and the widened apply
-- contract; on Seoul the column and trigger already exist, every statement is
-- guarded, and the apply RPC is server-only and disabled, so applying it there
-- is inert.

begin;

-- 1. Column parity ---------------------------------------------------------
-- Nullable and unconstrained on purpose. The compatibility project has no
-- gallery directory, so this is a carried identity, not a foreign key. Seoul
-- keeps its own `not null` + references constraint from the original migration.
...
comment on column public.exhibition_catalog_v2.gallery_id is
  'Gallery organization identity carried from the authoritative Seoul catalogue. On the compatibility project this is a mirrored value with no local directory.';

-- 2. Widened apply contract ------------------------------------------------
...
comment on function public.service_replace_legacy_mobile_catalog(jsonb, text, text) is
  'Service-role-only Seoul snapshot bridge for installed mobile readers. Both reader contracts preserve country identity, carried gallery identity, and canonical-v2 checksums; three-resource snapshots remain accepted during rollout.';
...
-- 3. Force one reconciliation ---------------------------------------------
-- The remembered hash refers to the pre-parity contract. Clearing it makes the
-- next scheduled pass reapply the current authoritative snapshot instead of
-- reporting "unchanged" against a catalogue that is now known to be stale.
update content_private.legacy_mobile_catalog_mirror_config
