...
class SplashControllerTest {
    @Test
    fun `happy path - theme then data ready then min time gates dismissal`() =
        runTest {
            val controller =
                SplashController(
                    minVisibleMs = 1500,
    ...
    @Test
    fun `slow data - data gate is the bottleneck`() =
        runTest {
            val controller =
                SplashController(
                    minVisibleMs = 1500,
    ...
    @Test
    fun `hard cap dismisses at 3s regardless of state`() =
        runTest {
            val controller =
                SplashController(
                    minVisibleMs = 1500,
    ...
    @Test
    fun `fast error - markDataReady on Error path also dismisses`() =
        runTest {
    ...
    @Test
    fun `idempotent - duplicate markDataReady calls do not crash`() =
        runTest {
            val controller =
                SplashController(
                    minVisibleMs = 1500,
    ...
    @Test
    fun `skipSplash flips visibility to false immediately`() =
        runTest {
            val controller =
                SplashController(
                    minVisibleMs = 1500,
