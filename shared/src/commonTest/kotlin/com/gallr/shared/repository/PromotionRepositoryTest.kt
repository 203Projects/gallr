...
class PromotionRepositoryTest {
    private val placement = PromotedExhibition(
    ...
    @Test
    fun `uses one installation key and coarse locality only`() = runTest {
        val calls = mutableListOf<List<String>>()
        val repository = PromotionRepositoryImpl(
            source = object : PromotionSource {
                override suspend fun fetch(key: String, cityKo: String, regionKo: String): PromotedExhibition? {
    ...
    @Test
    fun `does not create a key or call the service without locality`() = runTest {
        var keyReads = 0
        var sourceCalls = 0
        val repository = PromotionRepositoryImpl(
            source = object : PromotionSource {
            ...
            keyStore = object : PromotionInstallationKeyStore {
                override suspend fun getOrCreate(): String { keyReads += 1; return "unused" }
            },
        )
