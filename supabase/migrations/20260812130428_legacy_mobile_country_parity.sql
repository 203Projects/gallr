-- Keep the Singapore compatibility catalog byte-for-byte aligned after country
-- identity became part of the canonical checksum. This additive replacement
-- carries country_code through both installed-client reader contracts and
-- forces one fresh reconciliation.

-- Singapore is an isolated compatibility target and does not receive every
-- Seoul editorial migration. Upgrade only the two public contracts consumed by
-- installed clients so this migration is independently deployable there.
  ...
  for v_relation, v_name in
    values
      ...
      (
        ...
        'exhibition_catalog_v2_country_code_format'
      )
  loop
    if not exists (
      select 1
...
-- The checksum function serializes the complete projection row. Re-touching
-- country identity upgrades pre-country target checksums before the first new
-- snapshot arrives; on Seoul this is an idempotent checksum refresh.
update public.exhibition_catalog_v2
set country_code = country_code;

-- The compatibility receiver writes an authoritative Seoul snapshot directly
-- into the frozen target projection. Do not replace its country with any stale
-- canonical content that happens to remain in the compatibility database.
...
begin
  ...
  -- The frozen compatibility project intentionally has no requirement to
  -- carry the newer canonical editorial field. Its public row already has the
  -- Korea-first default and mirror-owned writes supply the source value.
  if not exists (
    select 1
    from information_schema.columns as column_record
    where column_record.table_schema = 'content'
      and column_record.table_name = 'exhibition_versions'
      and column_record.column_name = 'country_code'
  ) then
    return new;
  end if;
...
-- Replace the guarded receiver for the canonical-v2 reader used by iOS 1.7.4
-- and 1.7.5. The original three-resource payload remains accepted during the
-- Edge Function rollout window.
...
begin
  ...
  insert into public.exhibitions as target (
    id, name_ko, venue_name_ko, country_code, city_ko, region_ko, opening_date,
...
comment on function public.service_replace_legacy_mobile_catalog(jsonb, text, text) is
  'Service-role-only Seoul snapshot bridge for installed mobile readers. Both reader contracts preserve country identity and canonical-v2 checksums; three-resource snapshots remain accepted during rollout.';
...
-- An enabled target may contain stale rows or a remembered pre-country
-- snapshot. Force the next reconciliation to apply the expanded payload.
update content_private.legacy_mobile_catalog_mirror_config as config
