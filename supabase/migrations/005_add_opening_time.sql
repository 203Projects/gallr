...
--
-- The profiles/bookmarks migration was originally committed as `005b`, which
-- Supabase CLI does not recognize as a migration version. Production applied
-- those objects out of band after version 005, so its migration history has no
-- separate 005b entry. Keep the idempotent replay here, before migrations 006
-- and 007 depend on these objects, so clean databases reproduce production
-- without introducing an out-of-order synthetic version.
