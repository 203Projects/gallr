...
class DefaultMapScopeRegistry {
    ...
    fun childScopes(parent: MapScope, exhibitions: List<Exhibition>): List<MapScope> = when (parent.kind) {
        MapScopeKind.COUNTRY -> exhibitions
            .asSequence()
        ...
        MapScopeKind.CITY -> {
            val rowsByDistrict = exhibitionsInScope(parent, exhibitions)
        ...
        MapScopeKind.DISTRICT -> emptyList()
    }

    fun scope(scopeId: MapScopeId, exhibitions: List<Exhibition>): MapScope? {
    ...
    fun exhibitionsInScope(scope: MapScope, exhibitions: List<Exhibition>): List<Exhibition> =
    ...
    private fun cityScope(parent: MapScope, cityKey: String, rows: List<Exhibition>): MapScope {
    ...
    private fun boundsFor(rows: List<Exhibition>): GeoBounds? = GeoBounds.containing(
        rows.mapNotNull { row ->
...
private data class SeoulDistrict(val labelKo: String, val labelEn: String)
...
internal fun locationKey(preferred: String, fallback: String): String =
    preferred.ifBlank { fallback }
        .trim()
        .lowercase()
