...
class ExhibitionPaginationTest {
    ...
    @Test
    fun `loads complete collections at 999 1000 and 1001 row boundaries`() =
        runTest {
    ...
    @Test
    fun `continues after server-capped short pages until an empty page`() =
        runTest {
    ...
    @Test
    fun `retains featured and event filters for every page and integrity request`() =
        runTest {
    ...
    @Test
    fun `canonical same-id content change retries the entire fetch and recovers`() =
        runTest {
    ...
    @Test
    fun `permanent canonical same-id content mismatch retries once then fails closed`() =
        runTest {
            val staleRows = listOf(dto("same-id", contentChecksumSha256 = "a".repeat(64)))
            val currentRows = listOf(dto("same-id", contentChecksumSha256 = "b".repeat(64)))
            var collectionAttempts = 0
    ...
    @Test
    fun `verified empty canonical catalog succeeds with the empty catalog checksum`() =
        runTest {
            var pageCalls = 0
            var integrityCalls = 0
    ...
    @Test
    fun `canonical rows require a valid database content checksum`() =
        runTest {
    ...
    @Test
    fun `canonical integrity requires a valid catalog checksum without retry`() =
        runTest {
    ...
    @Test
    fun `sorts by opening date descending and preserves database order for ties`() =
        runTest {
            val rows =
                listOf(
    ...
    @Test
    fun `same-count membership change retries the entire fetch and recovers`() =
        runTest {
    ...
    @Test
    fun `permanent same-count membership mismatch throws without exposing a prefix`() =
        runTest {
    ...
    @Test
    fun `permanent count mismatch retries once then throws`() =
        runTest {
            val rows = listOf(dto("a"), dto("b"))
    ...
    @Test
    fun `malformed integrity values throw immediately without retry`() =
        runTest {
    ...
    @Test
    fun `integrity RPC failure propagates immediately without retry or partial result`() =
        runTest {
            val rows = listOf(dto("a"), dto("b"))
    ...
    @Test
    fun `rejects duplicate id inside a page`() =
        runTest {
            val error =
                assertFailsWith<IllegalStateException> {
    ...
    @Test
    fun `rejects a repeated cursor id on a later page`() =
        runTest {
            var calls = 0
    ...
    @Test
    fun `accepts database order that differs from Kotlin lexical order when integrity matches`() =
        runTest {
    ...
    @Test
    fun `invalid dto fails the complete request instead of being dropped`() =
        runTest {
    ...
    @Test
    fun `page two failure propagates without returning a partial prefix`() =
        runTest {
