...
private val thoughtComposerLog = AppLog.tagged("ThoughtComposer")

@Composable
fun ThoughtComposer(
    submitThought: suspend (String) -> Unit,
    lang: AppLanguage,
    onSubmitted: () -> Unit,
) {
    ...
    Column(
        ...
        Row(
            ...
            OutlinedButton(
                onClick = {
                    ...
                    scope.launch {
                        try {
                            submitThought(text.trim())
                            onSubmitted()
                        } catch (error: Exception) {
                            thoughtComposerLog.warn("submit_thought", error)
        ...
        if (submissionFailed) {
            ...
            GallrErrorMessage(
                message =
                    when (lang) {
                        AppLanguage.KO -> "감상을 게시하지 못했습니다. 다시 시도해주세요."
                        AppLanguage.EN -> "Couldn’t post your thought. Please try again."
                    },
            )
        }
    }
