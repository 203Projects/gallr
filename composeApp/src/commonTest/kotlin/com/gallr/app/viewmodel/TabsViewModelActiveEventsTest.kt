...
class TabsViewModelActiveEventsTest {
    ...
    private fun ev(
        ...
        brandColor = brandColor, ticketUrl = null, isActive = true,
    )
    ...
    private fun exh(
        ...
        isFeatured = false, latitude = latitude, longitude = longitude,
    ...
    @Test
    fun selected_event_filter_keeps_only_that_event() = runTest(dispatcher) {
        val (vm, _) = vm(
    ...
    @Test
    fun tapping_selected_event_filter_clears_it() = runTest(dispatcher) {
        val (vm, _) = vm(
    ...
    @Test
    fun orphaned_event_id_stays_visible_when_no_event_selected() = runTest(dispatcher) {
    ...
    @Test
    fun selected_event_clears_when_active_set_becomes_empty() = runTest(dispatcher) {
    ...
    @Test
    fun shrink_two_to_one_clears_removed_selected_event() = runTest(dispatcher) {
        val (vm, repo) = vm(
            events = listOf(ev("a", "2026-09-03", "2026-09-07"), ev("b", "2026-09-04", "2026-09-07")),
    ...
    @Test
    fun all_map_pins_include_colored_pins_for_each_active_event() = runTest(dispatcher) {
        val (vm, _) = vm(
            events = listOf(
